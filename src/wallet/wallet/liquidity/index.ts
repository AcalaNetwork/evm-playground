import { AnyApi, FixedPointNumber, forceToCurrencyName, MaybeCurrency, Token } from 'wallet/core';
...
import { sortTokenByName } from 'wallet/core/sort-token';
    ...
    (status: LiquidityPoolStatus = LiquidityPoolStatus.ENABLED): Observable<PoolInfo[]> => {
      ...
      return this.storages.tradingPairs().observable.pipe(
        ...
        switchMap((data) => {
          ...
          data.forEach((item) => {
            const [token1, token2] = [
              forceToCurrencyName(item[0].args[0][0]),
              forceToCurrencyName(item[0].args[0][1])
            ].sort(sortTokenByName);

            tokenCollections.add(token1);
            tokenCollections.add(token2);
          });

...
import { BalanceData, PresetCurrencies, TokenPriceFetchSource, TokenRecord, WalletConsts } from './type';
...
export class Wallet implements BaseSDK, TokenProvider {
  ...
  public getPresetCurrencies(): PresetCurrencies {
  ...
  public subscribePrice = memoize((token: MaybeCurrency, type?: PriceProviderType): Observable<FN> => {
    ...
    const presetCurrencies = this.getPresetCurrencies();
    const isLiquidToken = presetCurrencies?.liquidToken?.name === name;
    const stakingToken = presetCurrencies.stakingToken;
    ...
    if (name === 'AUSD') {
      return of(new FixedPointNumber(1));
    }

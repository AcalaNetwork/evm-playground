...
const DEFAULT_TOKEN_LIST: TokenListConfig = {
  [ChainType.ACALA]: {
    ...
    DOT: createTokenConfig('DOT', 'DOT', 10, new FixedPointNumber(0.01, 10)),
    LDOT: createTokenConfig('LDOT', 'LDOT', 10, new FixedPointNumber(0.05, 10)),
    ADAO: createTokenConfig('ADAO', 'ADAO', 10, new FixedPointNumber(0.05, 10)),
    SDAO: createTokenConfig('SDAO', 'SDAO', 10, new FixedPointNumber(0.05, 10))
  },
  [ChainType.KARURA]: {
...
class TokenList {
  public getToken(name: string, chain: string) {
    ...
    const data = DEFAULT_TOKEN_LIST?.ACALA?.[name];
